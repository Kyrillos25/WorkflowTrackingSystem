...
public sealed class WorkflowsDbContextFactory : IDesignTimeDbContextFactory<WorkflowsDbContext>
{
    public WorkflowsDbContext CreateDbContext(string[] args)
    {
        ...
        DbContextOptionsBuilder<WorkflowsDbContext> optionsBuilder = new DbContextOptionsBuilder<WorkflowsDbContext>()
            .UseNpgsql(
                connectionString,
                npgsql => npgsql.MigrationsHistoryTable(HistoryRepository.DefaultTableName, Schemas.Workflows))
            .UseSnakeCaseNamingConvention();

        return new WorkflowsDbContext(optionsBuilder.Options);
    }
}
